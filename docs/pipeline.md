...
### Фаза 0
- [x] Монорепо работает (`pnpm dev` запускает приложение)
- [x] shadcn/ui компоненты рендерятся корректно
- [x] Light/Dark тема переключается
- [x] CI/CD настроен (lint + build проходят)

### Фаза 1
- [x] Регистрация и логин работают
- [x] 2FA подключается и верифицируется
- [x] KYC-флоу проходится (упрощённый вариант)
- [x] Профиль отображает данные пользователя

### Фаза 2
- [x] Перевод по номеру телефона (СБП) работает
- [x] Чекаут проходит полный цикл
- [x] История операций отображается с фильтрами

### Фаза 3
- [x] Каталог товаров с сниппетами
- [x] Карточка товара (B2C + B2B)
- [x] Выбор доставки и трекинг

### Фаза 4
- [x] PFM-дашборд с категориями расходов
- [x] Программа лояльности начисляет и списывает баллы

### Фаза 5
- [x] Чат-бот ассистент отвечает на вопросы
- [x] Ассистент может инициировать действия (перевод, поиск)
