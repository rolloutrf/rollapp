...
Rollwish is a full-stack wishlist service: users collect wishes, share lists, follow friends, reserve gifts without spoiling the surprise, browse curated ideas, and run Secret Santa games.
...
Open `http://localhost:5173`. When `DATABASE_URL`/`PGHOST` is absent, the server uses an in-memory PostgreSQL-compatible demo database. Use **Try demo** or sign in as `demo@rollwish.ru` / `demo1234`.
...
- Managed PostgreSQL cluster `c9q11j9k294u5dmlk127`, database `rollwish`, user `rollwish_app`;
...
- database password stays in Connection Manager Lockbox secret `e6qf5vss5flop5pjagql`;
